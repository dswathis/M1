...
public class StatusController {
	...
	public ResponseEntity<Status> createStatus(@RequestBody @Valid StatusDto status,@PathVariable Long taskId) throws Exception {
		return new ResponseEntity<>(statuservice.createstatus(status,taskId), HttpStatus.CREATED);
	};
	
	@DeleteMapping({"/deleteStatusById/{id}"})
	public ResponseEntity<?> deleteStatus(@PathVariable Long id) throws ResourceNotFoundException {
	...
	public ResponseEntity<Status> findStatusById(@PathVariable Long id) throws ResourceNotFoundException {
		return new ResponseEntity<Status>(statuservice.findStatusById(id), HttpStatus.OK);
	}
	...
	public ResponseEntity<List<Status>> findstatusByUserId(@PathVariable Long id) throws ResourceNotFoundException {
		return new ResponseEntity<List<Status>>(statuservice.showTasksByUserId(id), HttpStatus.OK);
	}
//findTasksById
	
	@GetMapping("/my/{id}")
	public ResponseEntity<?> findSById(@PathVariable Long id) throws ResourceNotFoundException {
		return new ResponseEntity<Tasks>(statuservice.findTasksById(id), HttpStatus.OK);
	}
