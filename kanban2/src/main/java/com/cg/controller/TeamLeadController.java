...
public class TeamLeadController {
	...
	public ResponseEntity<TeamLead> createTeamLead(@RequestBody @Valid TeamLead user) throws Exception {
		return new ResponseEntity<>(teamleadService.createUser(user), HttpStatus.CREATED);

	};

	@PutMapping({ "/updateTeamLeadById/{uid}" })
	public ResponseEntity<TeamLead> updateTeamLead(@RequestBody  TeamLead user, @PathVariable Long uid)
	...
	public ResponseEntity<TeamLead> findTeamLeadById(@PathVariable Long id) throws ResourceNotFoundException {
		return new ResponseEntity<TeamLead>(teamleadService.findUserById(id), HttpStatus.OK);
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<TeamLead> updateteamLead(@RequestBody Map<String, Object> fields,@PathVariable Long id) throws ResourceNotFoundException{
        return ResponseEntity.ok(teamleadService.updateUser1(fields,id));
    }

	@PutMapping("/forgotPassword")
	public ResponseEntity<String> forgotPassword(@RequestBody ForgotPassDTO r) throws ResourceNotFoundException {
		Boolean isOtpVerify = adminAuthenticationService.verifyOtp(r.getEmail(), r.getOtp());
		var res = teamleadService.resetForgotPassword(r.getEmail(),r.getPassword());
		return ResponseEntity.ok("Password reset successfully");
		
    }
	@PostMapping("/sendEmail/{email}")
	public ResponseEntity<String> sendEmail(@PathVariable String email) throws ResourceNotFoundException {
		var res = adminAuthenticationService.sendEmail(email);
